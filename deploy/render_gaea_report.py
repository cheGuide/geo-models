...
# Style — standard light palette (matches generate_standardized_reports.py)
BG_FIG  = "#ffffff"
BG_AX   = "#ffffff"
GRID_C  = "#cccccc"
TEXT_C  = "#000000"
C_GAEA  = "#1E90FF"   # dodger blue — val curve
C_GEO   = "#27AE60"   # green
C_KOP   = "#FF0000"   # red — Kopernik baseline
C_GOLD  = "#FF8C00"   # dark orange — training / primary
C_CYAN  = "#27AE60"
C_PURP  = "#9B59B6"
C_BASE  = "#FF0000"   # 3545 m line
...
def panel_multitask_loss(ax, df: pd.DataFrame) -> None:
    ...
    ax.legend(facecolor="white", edgecolor=GRID_C, labelcolor=TEXT_C, fontsize=6.5,
              loc="upper right")
...
def panel_cdf(ax, gaea_errors: np.ndarray) -> None:
    ...
    style(ax, "Error CDF: GAEA vs Baselines", "Haversine Error (m)",
          "Cumulative Frames (%)")
    ax.legend(facecolor="white", edgecolor=GRID_C, labelcolor=TEXT_C, fontsize=5.8,
              loc="lower right")
...
def _synthetic_attention_map(ax, reason: str = "") -> None:
    ...
    if reason:
        ax.text(0.02, 0.02, f"[schematic — {reason[:60]}]",
                transform=ax.transAxes, color="#555555", fontsize=5, alpha=0.6)
