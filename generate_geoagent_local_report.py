...
if plot_epochs:
    ...
    ax.plot(plot_epochs[:n_real], plot_losses[:n_real],
            "o-", color=ROYAL_BLUE, lw=2, ms=7, zorder=3, label="Measured loss")
    # Projected tail
    if len(plot_epochs) > n_real:
        ax.plot(plot_epochs[n_real-1:], plot_losses[n_real-1:],
                "--", color=ROYAL_BLUE, lw=1.5, alpha=0.55, zorder=2,
...
ax.set_title("[1,1]  LoRA Training Loss — GeoAgent TTK")


# ── [0,1] Local Mean & Median vs Checkpoint ────────────────────────────────────
ax = axes[0][1]
style_ax(ax)
# Checkpoints 1-4 from CSV
ax.plot(steps_csv, vm_local_approx, "o-", color=ROYAL_BLUE, lw=2, ms=6,
        label="Mean Error", zorder=3)
ax.plot(steps_csv, vmed_local_approx, "s--", color=ACCENT_GRN, lw=2, ms=6,
        label="Median Error", zorder=3)
# Add actual benchmark point
ax.plot(steps_ext[-1], local_mean,   "D", color=ROYAL_BLUE, ms=10, zorder=4,
        markeredgecolor="white", markeredgewidth=1.5)
ax.plot(steps_ext[-1], local_median, "D", color=ACCENT_GRN, ms=10, zorder=4,
        markeredgecolor="white", markeredgewidth=1.5)
ax.annotate(f"Mean\n{local_mean/1000:.2f} km", (steps_ext[-1], local_mean),
            xytext=(-55, 8), textcoords="offset points",
            fontsize=7.5, color=ROYAL_BLUE, fontweight="bold",
            arrowprops=dict(arrowstyle="-", color=ROYAL_BLUE, lw=0.8))
...
ax.set_xlabel("Checkpoint (step)", labelpad=4)
ax.set_ylabel("Error (m)", labelpad=4)
ax.legend(fontsize=7.5, loc="upper left")
ax.set_title("[1,2]  Mean & Median Distance")


# ── [0,2] Data Retention Rate vs Checkpoint ───────────────────────────────────
ax = axes[0][2]
style_ax(ax)
bar_colors = [ROYAL_BLUE] * len(steps_csv) + [ACCENT_GOLD]
...
ax.set_xticklabels(step_labels, fontsize=7.5)
ax.set_ylabel("Coverage Rate (%)", labelpad=4)
ax.set_ylim(0, 115)
ax.axhline(retention, color=ACCENT_GOLD, lw=1.5, ls="--", alpha=0.7, zorder=2)
ax.set_title("[1,3]  Within-City Coverage Rate")
...
if n_local:
    ...
    ax.plot(sorted_err, cdf, color=ROYAL_BLUE, lw=2, zorder=3)
    ax.fill_between(sorted_err, 0, cdf, alpha=0.12, color=ROYAL_BLUE, zorder=2)
    # Kopernik reference line
    kop_pct = float(np.interp(KOPERNIK_M, sorted_err, cdf))
    ax.axvline(KOPERNIK_M, color=ACCENT_RED, lw=1.8, ls="--", zorder=4)
    ax.text(KOPERNIK_M + 150, 10, f"Kopernik\n{KOPERNIK_M} m ({kop_pct:.0f}%)",
            fontsize=7.5, color=ACCENT_RED)
    # Mark median
    ax.axvline(local_median, color=ACCENT_GRN, lw=1.5, ls=":", zorder=4)
    ax.text(local_median + 150, 55,
            f"Median\n{local_median:.0f} m",
            fontsize=7.5, color=ACCENT_GRN)
...
if n_local:
    ...
    counts, edges, patches = ax.hist(errors_local, bins=n_bins,
                                     ...
                                     zorder=3)
    # Colour bars below Kopernik threshold in green
    for patch, left, right in zip(patches, edges[:-1], edges[1:]):
        if right <= KOPERNIK_M:
            patch.set_facecolor(ACCENT_GRN)
            patch.set_alpha(0.80)
    ax.axvline(KOPERNIK_M, color=ACCENT_RED, lw=2.5, ls="--",
               zorder=5, label=f"Kopernik baseline: {KOPERNIK_M} m")
    ax.axvline(local_mean, color=ACCENT_GOLD, lw=2, ls="-.",
               zorder=4, label=f"Local Mean: {local_mean:.0f} m")
    ax.axvline(local_median, color=ACCENT_GRN, lw=2, ls=":",
               zorder=4, label=f"Local Median: {local_median:.0f} m")
    ax.xaxis.set_major_formatter(FuncFormatter(lambda x, _: f"{x/1000:.0f}km"))
    ax.set_xlabel("Haversine Error", labelpad=4)
    ax.set_ylabel("Count", labelpad=4)
    ax.legend(fontsize=7.5, loc="upper right")
ax.set_title("[2,2]  Error Histogram | Kopernik @ 3 545 m")
...
if n_local:
    ...
    # Ideal calibration line (inversely: high conf → low error)
    cmin, cmax = 0.25, 0.95
    emin, emax = errors_local.min(), errors_local.max()
    cal_x = np.array([cmin, cmax])
    cal_y = emax + (emin - emax) * (cal_x - cmin) / (cmax - cmin)
    ax.plot(cal_x, cal_y, "--", color=ACCENT_RED, lw=1.5, alpha=0.6,
            zorder=2, label="Ideal calibration")
    ax.axhline(KOPERNIK_M, color=ACCENT_RED, lw=1.2, ls=":", alpha=0.5, zorder=2)
    ax.yaxis.set_major_formatter(FuncFormatter(lambda x, _: f"{x/1000:.1f}km"))
    ax.set_xlabel("Predicted Confidence (from Uncertainty field)", labelpad=4)
    ax.set_ylabel("Actual Error", labelpad=4)
    ax.set_xticks([0.30, 0.60, 0.90])
    ax.set_xticklabels(["High\nunc.\n(0.30)", "Med\nunc.\n(0.60)", "Low\nunc.\n(0.90)"], fontsize=7)
    ax.legend(fontsize=7, loc="upper right")
ax.set_title("[2,3]  Confidence Calibration — Predicted vs Actual Error")


# ── Super-title and annotation banner ─────────────────────────────────────────
label_str = (
    f"Mean: {local_mean:.0f} m  |  "
    f"Median: {local_median:.0f} m  |  "
    f"N: {n_local}"
)

fig.suptitle(
    "GeoAgent — Moscow TTK-10k",
    fontsize=13, fontweight="bold", color=ROYAL_BLUE, y=0.97
)

fig.text(
    0.5, 0.017,
    f"Annotation: 'Evaluating GeoAgent localization accuracy on the Moscow TTK dataset'  |  {label_str}",
